...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tests-186%20passing-brightgreen?style=for-the-badge&logo=pytest" />
...
This repository is a fully working **Proof-of-Concept** for the NeuroSim proposal, demonstrating all pipeline modules with **186 passing unit tests** and **4 executed Jupyter notebooks**. It directly addresses the two physics-constrained benchmark challenges raised during the INCF mentor review.
...
│   ├── graphnet.py            # GraphNet MVAR: L1 + graph Laplacian regularization
│   └── wilson_cowan.py        # Wilson-Cowan simulator + MVAR nonlinear validation
├── control/
...
└── ingestion/
    ├── bids_loader.py         # BIDS-compliant subject file indexing (fMRIPrep derivatives)
    ├── parcellation.py        # Schaefer atlas parcellation + synthetic timeseries generator
    └── signal_cleaning.py     # OLS confound regression, linear detrending, tSNR QC
```
...
| `test_frobenius_recovery.py`   | 19      | Ground-truth recovery, eigenvalue structure, stabilization |
| `test_ingestion.py`            | 22      | BIDS loader, parcellation, signal cleaning, tSNR QC       |
| `test_graphnet.py`             | 18      | GraphNet MVAR, Laplacian construction, sparsity           |
| `test_wilson_cowan.py`         | 17      | WC simulator, MVAR nonlinear validation, stability        |
...
| **GraphNet MVAR (L1 + graph Laplacian)**            | ✅ Complete | `connectivity/graphnet.py`      |
| **Wilson-Cowan nonlinear validation**               | ✅ Complete | `connectivity/wilson_cowan.py`  |
...
| **BIDS ingestion + signal cleaning + tSNR QC**      | ✅ Complete | `ingestion/`                    |
| **Full pipeline demo script**                       | ✅ Complete | `run_demo.py`                   |
| Unit test suite (186 tests)                         | ✅ Complete | `tests/`                        |
